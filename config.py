...
BG_COLOR = (255, 255, 255)    # Цвет фона (темно-серый)
...
# Скорости объектов
BULLET_SPEED = 7              # Увеличенная скорость пуль (было 7)
MINE_SPEED = 1.4               # Базовая скорость мин

# Временные параметры
START_DELAY = 180              # Задержка перед началом движения мин (в кадрах)
ENEMY_REACTION_TIME = 355555550       # Уменьшенное время реакции врага (было 20)

# Параметры спавна
MIN_SPAWN_DISTANCE = 250       # Минимальное расстояние от центра для спавна мин
...
STRATEGY_PARAMS = {
    ...
    "zigzag": {
        "speed_multiplier": 1.0,    # Множитель скорости для зигзагообразного движения
        "period": 15,               # Период зигзага (в кадрах)
        "amplitude": 1.5,           # Амплитуда зигзага (в радианах)
    },
    "spiral": {
